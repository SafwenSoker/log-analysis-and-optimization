...
_SEVERITY_MAP: dict[ErrorCategory, Severity] = {
    ...
    ErrorCategory.JVM_ERROR: Severity.CRITICAL,
    ...
    ErrorCategory.MAVEN_DEPENDENCY_ERROR: Severity.HIGH,
    ErrorCategory.SELENIUM_DRIVER_ERROR: Severity.HIGH,
    ErrorCategory.SELENIUM_UI_FAILURE: Severity.MEDIUM,
...
_CATEGORY_LABELS: dict[ErrorCategory, str] = {
    ...
    ErrorCategory.TIMEOUT: "Build step timed out",
    ErrorCategory.SELENIUM_UI_FAILURE: "Selenium UI element not found (XPath)",
    ErrorCategory.SELENIUM_DRIVER_ERROR: "Selenium ChromeDriver not found or crashed",
    ErrorCategory.MAVEN_DEPENDENCY_ERROR: "Maven dependency resolution failed",
    ErrorCategory.JVM_ERROR: "Java Virtual Machine failed to start",
...
_RECOMMENDATIONS: dict[ErrorCategory, list[str]] = {
    ...
    ErrorCategory.SELENIUM_UI_FAILURE: [
        "Check if the UI element targeted by the XPath still exists — a recent deployment may have changed the DOM.",
        "Review the Selenium screenshot (if captured) to see the actual page state at the time of failure.",
        "Run the failing scenario in isolation to confirm reproducibility.",
        "Check if the application was slow to load — increase the implicit wait if needed.",
    ],
    ErrorCategory.SELENIUM_DRIVER_ERROR: [
        "Verify that ChromeDriver is installed on the Jenkins agent and matches the installed Chrome version.",
        "Check that the PATH on the Jenkins agent includes the ChromeDriver executable.",
        "Restart the Jenkins agent if ChromeDriver was left in a zombie state by a previous build.",
        "Consider pinning ChromeDriver to a specific version to avoid version mismatch after Chrome auto-updates.",
    ],
    ErrorCategory.MAVEN_DEPENDENCY_ERROR: [
        "Check connectivity from the Jenkins agent to the Nexus/Maven Central repository.",
        "Verify that the parent POM version referenced in pom.xml exists in the repository.",
        "Run `mvn dependency:resolve` locally to reproduce the error.",
        "Check if a recent change to pom.xml introduced an invalid or missing dependency.",
    ],
    ErrorCategory.JVM_ERROR: [
        "Check available memory on the Jenkins agent — the JVM may have failed due to insufficient heap.",
        "Review Jenkins agent logs for OOM (Out Of Memory) errors around the build time.",
        "Check that JAVA_HOME is correctly set and points to a valid JDK installation.",
        "Try reducing Maven heap settings (`MAVEN_OPTS=-Xmx512m`) if memory is constrained.",
    ],
